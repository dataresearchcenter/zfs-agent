...
- **`cli.py`** – Typer entry point; single flat command (`zfs-agent --socket ... --pool ...`, no subcommand). Resolves flags against `Settings` env vars and calls `serve()`; no socket handling of its own.
